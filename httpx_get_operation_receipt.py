...
open_credit_account_payload = {"userId": create_user_response_data['user']['id']}
...
get_operation_receipt_response = httpx.get(
    f'http://localhost:8003/api/v1/operations/{make_purchase_operation_response_data["operation"]["id"]}'
)
get_operation_receipt_response_data = get_operation_receipt_response.json()

print('Purchase operation receipt: ', get_operation_receipt_response_data)
